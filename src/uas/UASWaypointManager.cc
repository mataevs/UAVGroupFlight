...
void UASWaypointManager::writeWaypoints() {
	this->sendWaypointClearAll();

	for (int i = 0; i < waypointsEditable.count(); i++) {
		Waypoint* wp = waypointsEditable.at(i);
		QString state("@INSERT-1:%1,%2@%3");
		state = state.arg(wp->getLongitude()).arg(wp->getLatitude()).arg(wp->getAltitude() * 3.2808);

		qDebug() << "Waypoint to be transmitted:" << state;

		this->uas->transmitHilRouteManagerCommand(state, NULL);
	}
}
...
void UASWaypointManager::sendWaypointClearAll()
{
	this->uas->transmitHilRouteManagerCommand(QString("@CLEAR"), NULL);
}
...
void UASWaypointManager::sendWaypointSetCurrent(quint16 seq) {
	QString state("@JUMPTO%1");
	state = state.arg(seq);

	qDebug() << "Change current waypoint received." << state;
